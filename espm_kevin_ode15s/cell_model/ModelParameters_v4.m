...
param.Rs_n = 10^(-6.8017);            % Anode particle radius
param.Rs_p = 10^(-6.8670);            % Cathode particle radius         
param.A = 0.1991;     % Area (averaged from anode/cathode area from Chen et al. 2020)
param.Ln = 85.2e-06;                % Anode thickness (from Chen et al. 2020)
param.Lp = 75.6e-06;                % Cathode thickness (from Chen et al. 2020)
param.epsilon_n = 0.4464;             % Anode solid phase volume fraction (1-poro)
param.epsilon_p = 0.3850;             % Cathode solid phase volume fraction (1-poro)
param.theta100_n = 0.8383;            % Anode stoichiometry at 100% SOC
param.theta100_p = 0.2743;            % Cathode stoichiometry at 100% SOC
param.theta0_n = 0.0264;              % Anode stoichiometry at 0% SOC
param.theta0_p = 0.8845;              % Cathode stoichiometry at 0% SOC
...
param.Dsn_ref = 10^(-12.3546);   % Anode diffusion coefficient
param.Dsp_ref = 10^(-17.8248);   % Cathode diffusion coefficient

% Reaction rates
param.kn_ref = 10^(-1.8576)/param.F;     % Anode reaction rate constant 
param.kp_ref = 10^(-1.6913)/param.F;     % Cathode reaction rate constant

% Introduce stochastic variation to Lumped Contact Resistance
param.R_l_ref = 0.0189;
...
% Values from Tanim et al. 2014 (https://doi.org/10.1115/1.4028154), Table 1
param.Ea_Dsp = 25000;
param.Ea_Dsn = 30000;
param.Ea_kp = 58000;
param.Ea_kn = 92000;
