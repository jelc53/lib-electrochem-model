function [dx_dt] = Module_ode_sundials(x_in,input_crt, param)
import casadi.*
global A_csn A_csp
...
x_pes=300*ones(tot_state_num,1);
...
T_cell = x_pes(index_ce+1:index_ce+param.Nc*2);  %Cell temperatures (surface + core)
...
L_sei = x_pes(index_thermal+1:index_thermal+param.Nc);                  %SEI Layer Thickness
Q = x_pes(index_thermal+param.Nc+1:index_thermal+2*param.Nc);           %ODE-updated Capacity   
aina_n = x_pes(index_thermal+2*param.Nc+1:index_thermal+3*param.Nc,:);  %Inactive area evolution, anode
aina_p = x_pes(index_thermal+3*param.Nc+1:index_thermal+4*param.Nc,:);  %Inactive area evolution, cahode
c_sei = x_pes(index_thermal+4*param.Nc+1:index_thermal+5*param.Nc,:);   %SEI concentration
c_li = x_pes(index_thermal+5*param.Nc+1:index_thermal+6*param.Nc,:);    %Plated li concentration
L_film = x_pes(index_thermal+6*param.Nc+1:index_thermal+7*param.Nc,:);  %Film Layer Thickness

%Define SEI Solvent Concentration States
Csolv = x_pes(index_aging+1:end); %Solvent Concentration in SEI 
...
% T_core(param.Nc,1) = 0;
% T_surf(param.Nc,1) = 0;
theta_surf_n=SX.sym('theta_surf_n',param.Nc);
theta_surf_p=SX.sym('theta_surf_n',param.Nc);
for i = 1:param.Nc
...
%Allocate Memory
ce_n_avg=SX.sym('ce_n_avg',param.Nc,1);
ce_s_avg=SX.sym('ce_s_avg',param.Nc,1);
ce_p_avg=SX.sym('ce_p_avg',param.Nc,1);
ce_all=SX.sym('ce_p_avg',(param.Nx_n+param.Nx_s+param.Nx_p),param.Nc);
ce_all_avg=SX.sym('ce_p_avg',param.Nc,1);
...
De_n_minus0_5=SX.sym('De_n_minus0_5',param.Nc*param.Nx_n,1);
De_n_plus0_5=SX.sym('De_n_minus0_5',param.Nc*param.Nx_n,1);
De_s_minus0_5=SX.sym('De_n_minus0_5',param.Nc*param.Nx_s,1);
De_s_plus0_5=SX.sym('De_n_minus0_5',param.Nc*param.Nx_s,1);
De_p_minus0_5=SX.sym('De_n_minus0_5',param.Nc*param.Nx_p,1);
De_p_plus0_5=SX.sym('De_n_minus0_5',param.Nc*param.Nx_p,1);
...
De_eff_n_s=SX.sym('De_eff_n_s',param.Nc);
De_eff_s_p=SX.sym('De_eff_s_p',param.Nc);
for i = 1:param.Nc
...
i_s_dummy=SX.sym('i_s_dummy',param.Nc*(param.Nr-1));
i_lpl_dummy = SX.sym('i_lpl_dummy',param.Nc*(param.Nr-1));
for i = 1:param.Nc
...
dCen_interf_ns=SX.sym('dCen_interf_ns',param.Nc);
dCes_interf_ns=SX.sym('dCes_interf_ns',param.Nc);
dCes_interf_sp=SX.sym('dCes_interf_sp',param.Nc);
dCep_interf_sp=SX.sym('dCep_interf_sp',param.Nc);
