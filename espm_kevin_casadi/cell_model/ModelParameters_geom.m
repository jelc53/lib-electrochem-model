...
param.Dsn_ref = 10^(log10(1.74e-15));   % Anode diffusion coefficient (from Chen et al. 2020)
param.Dsp_ref = 10^(log10(1.48e-15));   % Cathode diffusion coefficient (from Chen et al. 2020)
...
param.epsilon_n = x(4);             % Anode solid phase volume fraction (1-poro)
param.epsilon_p = x(5);             % Cathode solid phase volume fraction (1-poro)
param.theta100_n = x(6);            % Anode stoichiometry at 100% SOC
param.theta100_p = x(7);            % Cathode stoichiometry at 100% SOC
param.kn_ref = 6.48e-7/param.F;     % Anode reaction rate constant (from Chen et al. 2020)
param.kp_ref = 3.42e-6/param.F;     % Cathode reaction rate constant (from Chen et al. 2020)
param.theta0_n = x(8);              % Anode stoichiometry at 0% SOC
param.theta0_p = x(9);              % Cathode stoichiometry at 0% SOC
...
% param.Ea_Dsp = 25000; % KEVIN: Where are these values from???
% param.Ea_Dsn = 40000;
% param.Ea_kp = 30000;
% param.Ea_kn = 30000;
