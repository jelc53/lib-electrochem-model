function [V_cell, R_l, T_core, T_surf,soc_bulk_n, soc_bulk_p, cs_n, cs_p,...
          ...
          te, xe, ie] = Module_Run(x_initial,param,method_flag)
...
if method_flag==1  %===========Using FVM method for solid phase
%% Finite Volume alternative for solid phase LX version
event_formatted = @(t,x) physical_event_function_tk(t,x,param); 
options=odeset('RelTol',reltol,'AbsTol',abstol,'Events',event_formatted);
%===============fvm
...
u_initial = x_initial(1:2*(param.Nr-1)).*[r_n,r_p]';
tspan=t_out;
[t_out,u_out] = ode15s(@(t_out, u_out) fvm_ode_lx5(t_out, u_out, param), tspan, u_initial, options);

cell_out = u_out' ./[r_n,r_p]';
...
%=================Calculate surface concentraion
...
I_dummy = param.I_data(1:size(u_out,1)); %Store exact current profiles used in I_dummy
param.I_data = I_dummy;
param.t_data = t_out;
end
...
for j = 1:size(cs,2)  %NOTE: check length(cs) is a bug!
    ...
    for i = 1:param.Nc
        if method_flag==0  %===========FDM method
        theta_surf_n(i,j) = cs_n(i*(param.Nr-1),j)/param.c_n_max;
        theta_surf_p(i,j) = cs_p(i*(param.Nr-1),j)/param.c_p_max;
        elseif method_flag==1 %===========FVM method
        theta_surf_n(i,j) = new_cssurn(j)/param.c_n_max;
        theta_surf_p(i,j) = new_cssurp(j)/param.c_p_max;
        end
...
[soc_bulk_n,soc_bulk_p] = soc_calculation_fvm(cs_n,cs_p,param);   
end
