...
import pandas as pd
dis_Nr=[5,10,20,40,60,80]
# dis_Nr=[5]
tot_sim_time=3000
tot_cspsur=np.zeros((tot_sim_time+1,np.shape(dis_Nr)[0]))
tot_csnsur=np.zeros((tot_sim_time+1,np.shape(dis_Nr)[0]))
tot_cspave=np.zeros((tot_sim_time+1,np.shape(dis_Nr)[0]))
tot_csnave=np.zeros((tot_sim_time+1,np.shape(dis_Nr)[0]))
tot_V=np.zeros((tot_sim_time+1,np.shape(dis_Nr)[0]))
tot_t=np.zeros((tot_sim_time+1,np.shape(dis_Nr)[0]))

for i in range(0,np.shape(dis_Nr)[0]):
    ...
    parameter_values['Current function [A]'] = -4.87*1
    ...
    var_pts = {var.x_n: 15, var.x_s: 10, var.x_p: 15, var.r_n: dis_Nr[i], var.r_p: dis_Nr[i]}
...
np.savetxt('pybamm_t_single.txt',tot_t)
np.savetxt('pybamm_V_single.txt',tot_V)
np.savetxt('pybamm_cspsur_single.txt',tot_cspsur)
np.savetxt('pybamm_csnsur_single.txt',tot_csnsur)
np.savetxt('pybamm_cspave_single.txt',tot_cspave)
np.savetxt('pybamm_csnave_single.txt',tot_csnave)
